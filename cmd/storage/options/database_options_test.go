...
func TestDatabaseOptions_AddFlags(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer func() {
				_ = os.Setenv("STORAGE_DB_PORT", originalDBPortEnv)
				_ = os.Setenv("STORAGE_AUTO_MIGRATE", originalAutoMigrateEnv)
			}()

			if tt.setDBPortEnv {
				_ = os.Setenv("STORAGE_DB_PORT", tt.dbPortEnvVal)
			} else {
				_ = os.Unsetenv("STORAGE_DB_PORT")
			}

			if tt.setAutoMigrate {
				_ = os.Setenv("STORAGE_AUTO_MIGRATE", tt.autoMigrateVal)
			} else {
				_ = os.Unsetenv("STORAGE_AUTO_MIGRATE")
			}
