...
func TestDocumentRepository_ListByUserID(t *testing.T) {
	...
	// GORM will select all fields from DocumentModel including BaseModel fields
	...
	// Match the SELECT for documents by user_id exactly as generated by GORM.
	...
	// Expect empty result set for a different user (no error, just empty slice)
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT * FROM "documents" WHERE user_id = $1 AND "documents"."deleted_at" IS NULL`)).
...
func TestDocumentRepository_GetByID(t *testing.T) {
	...
	// Successful fetch; match the exact SELECT GORM generates.
...
func TestDocumentRepository_Delete(t *testing.T) {
	...
	// GORM soft-deletes by updating deleted_at, not issuing a raw DELETE.
